...
if(!checkPermission($group_id, 'exam_type', 'lists')){
...
require _WEB_PATH_ROOT.'/admin/exam_type/model/board.php';
...
?>
    <?php view($view, 'admin', 'exam_type'); ?> 
</div>


<div>

    <h5>Danh sách Loại bài kiểm tra</h5>
    <hr>
    ...
    <thead>
        <tr>
            ...
            <?php if(checkPermission($group_id, 'exam_type', 'fix')): ?>
            <th width="10%" class="board_th">Sửa</th>
            <?php endif; ?>
            <?php if(checkPermission($group_id, 'exam_type', 'delete')): ?>
    ...
    foreach ($allExamType as $key => $item): 
    extract($item);
