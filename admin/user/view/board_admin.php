...
if(!checkPermission($group_id, 'admin', 'lists_admin')){
...
$numberUser = getCountRows("SELECT u.id from user AS u INNER JOIN groups AS g ON u.id_group=g.id WHERE id_group<>'7' $filter");
...
$allUser = getRaw("SELECT u.*, g.name AS 'g_name' from user AS u INNER JOIN groups AS g ON u.id_group=g.id WHERE id_group<>'7' $filter LIMIT $limitS, $limitE");
...
<div class="container_my">
    ...
    <?php if(checkPermission($group_id, 'admin', 'add_admin')): ?>
    <a href="?module=user&action=add_admin" class="btn btn-primary mb-3">Thêm <i class="fa fa-plus mx-1"></i></a>
    <?php endif; ?>
    <form action="" method="get" class="mx-0 row">

        <input type="hidden" name="module" value="<?php echo $module; ?>">
        <input type="hidden" name="action" value="lists_admin">
    ...
    <table class="w-100">
        <thead>
            <tr>
                ...
                <?php if(checkPermission($group_id, 'admin', 'approve_admin')): ?>
                <th width="5%" class="board_th">Trạng thái</th>
                <?php endif; ?>
                <th width="5%" class="board_th">Quyền</th>
                <?php if(checkPermission($group_id, 'admin', 'fix_admin')): ?>
        ...
        ?>
            ...
            <tr>
                ...
                <?php if(checkPermission($group_id, 'admin', 'approve_admin')): ?>
                <td class="board_td text-center"><a href="?module=user&action=apprive_admin&id=<?php echo $id; ?>" class="btn btn-<?php echo !empty($status)?'success':'danger'; ?>"><?php echo !empty($status)?'Duyệt':'Chưa duyệt'; ?></a></td>
                <?php endif; ?>
                <td class="board_td text-center"><a href=""><?php echo $g_name; ?></a></td>
                <?php if(checkPermission($group_id, 'admin', 'fix_admin')): ?>
                <td class="board_td text-center">
                    <a href="<?php echo '?module=user&action=fix_admin&id='.$id; ?>" class="btn btn-warning"><i class="fa fa-wrench"></i></a>
                </td>
                <?php endif; ?>
                <?php if(checkPermission($group_id, 'admin', 'delete_admin')): ?>
                <td class="board_td text-center">
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                <li class="page-item d-<?php echo $page == 1 ? 'none' : 'block'; ?>">
                    <a class="page-link" href="<?php echo "?module=$module&action=lists_admin$urlFilter&page=$back"; ?>" aria-label="Previous">
                ...
                ?>
                    ...
                    <li class="page-item <?php echo $page == $i ? 'active' : ''; ?>"><a class="page-link" href="<?php echo "?module=$module&action=lists_admin$urlFilter&page=$i"; ?>"><?php echo $i; ?></a></li>
                ...
                <li class="page-item d-<?php echo $page == $totalPage ? 'none' : 'block'; ?>">
                    <a class="page-link" href="<?php echo "?module=$module&action=lists_admin$urlFilter&page=$next"; ?>" aria-label="Next">
